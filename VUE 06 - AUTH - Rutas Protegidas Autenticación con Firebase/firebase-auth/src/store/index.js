    ...
    actions: {
        ...
        crearUsuario({
            ...
            firebase.auth().createUserWithEmailAndPassword(payload.email, payload.pass).then(res => {
                ...
                // ?Crear una colección
                db.collection(res.user.email).add({nombre: 'tarea de ejemplo'}).then(() => {
                    router.push({name: 'inicio'})
                }).catch(err => {
                    console.log(err);
                    commit('setError', err.code)
                })
                //?Finaliza la creacion de la coleccion
            }).catch(err => {
                console.log(err);
                commit('setError', err.code)
            })
        ...
        editarTarea({
            ...
            const usuario = firebase.auth().currentUser
            db.collection(usuario.email).doc(tarea.id).update({nombre: tarea.nombre}).then(() => {
                router.push({name: 'inicio'})
            })
        },
        agregarTarea({
            ...
            const usuario = firebase.auth().currentUser
            db.collection(usuario.email).add({nombre: nombre}).then(doc => {
                console.log(doc.id);
                router.push({name: 'inicio'})
                commit("cargarFirebase", false)
            })
        ...
        eliminarTarea(
            ...
            const usuario = firebase.auth().currentUser
            db.collection(usuario.email).doc(id).delete().then(() => {
